...
function createBeam(width: number, height: number, isDarkMode: boolean): Beam {
  ...
  const hueBase = isDarkMode ? 325 : 332;
  const hueRange = isDarkMode ? 24 : 18;
...
export default function BeamsBackground({
  ...
  useEffect(() => {
    ...
    function resetBeam(beam: Beam, index: number, totalBeams: number) {
      ...
      const hueBase = isDarkModeRef.current ? 325 : 332;
      const hueRange = isDarkModeRef.current ? 24 : 18;
    ...
    function drawBeam(ctx: CanvasRenderingContext2D, beam: Beam) {
      ...
      const saturation = isDarkModeRef.current ? "84%" : "78%";
      const lightness = isDarkModeRef.current ? "72%" : "76%";
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="pointer-events-none fixed inset-0 bg-pink-50/35"
        style={{
